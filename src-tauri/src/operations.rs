...
/// Read-only preflight uses the same canonical-path policy as the mutation.
#[tauri::command]
pub(crate) async fn deletion_path_warning(
  ...
  tauri::async_runtime::spawn_blocking(move || {
    ...
    let path = if let Some(id) = id {
      let map = state.map.lock().map_err(|error| error.to_string())?;
      let source = map.get(&id).ok_or("File not found")?;
      local_source_path(source).map(Path::to_path_buf)
    } else {
      folder_path.map(PathBuf::from)
    };
    Ok(path.as_deref().and_then(protected_path_reason))
  })
  .await
  ...
  tauri::async_runtime::spawn_blocking(move || {
    ...
    match source {
      ManagedFileSource::LocalPath(path) => {
        ...
        let outcome = execute_authorized_local_operation(
          &path,
          operation,
          &state.trash_dir,
          allow_unsafe,
          allow_permanent_delete.unwrap_or(false),
        );
        let _ = append_operation_journal(
          ...
          Some(if allow_unsafe { "manual" } else { "safe" }.into()),
      ...
      ManagedFileSource::AndroidDocument(source) => {
        ...
        match mode {
          ...
          TrashMode::Permanent => {
            if !allow_unsafe && !allow_permanent_delete.unwrap_or(false) {
              map.insert(id, ManagedFileSource::AndroidDocument(source.clone()));
...
#[tauri::command]
pub(crate) async fn trash_folder(
  app_handle: AppHandle,
  folder_path: String,
  ...
  tauri::async_runtime::spawn_blocking(move || {
    ...
    let backup = execute_authorized_local_operation(
      &source_path,
      operation,
      &state.trash_dir,
      allow_unsafe,
...
#[tauri::command]
pub(crate) async fn restore_folder(
  app_handle: AppHandle,
  source: String,
  destination: String,
  files: Vec<FolderTrashEntry>,
...
#[tauri::command]
pub(crate) async fn apply_action_batch(
  app_handle: AppHandle,
...
#[tauri::command]
pub(crate) async fn undo_action_batch(
  app_handle: AppHandle,
  batch_id: String,
